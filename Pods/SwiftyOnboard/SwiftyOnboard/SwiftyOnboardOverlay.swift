...
open class SwiftyOnboardOverlay: UIView {
    ...
    open var continueButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(tapActionCon(_:)), for: .touchUpInside)
        return button
    }()
    ...
    @objc func tapActionCon(_ sender: UIButton) {
        self.didTapContinue?()
    }
    
    
    open var didTapContinue: (() -> ())?
    ...
    open func set(style: SwiftyOnboardStyle) {
        switch style {
        case .light:
            continueButton.setTitleColor(UIColor.init(red: 230, green: 194, blue: 49, alpha: 1), for: .normal)
            skipButton.setTitleColor(UIColor.init(red: 230, green: 194, blue: 49, alpha: 1), for: .normal)
            pageControl.currentPageIndicatorTintColor = UIColor.init(red: 230, green: 194, blue: 49, alpha: 1)
