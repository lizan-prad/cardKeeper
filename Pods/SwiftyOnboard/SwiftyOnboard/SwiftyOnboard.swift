    ...
    fileprivate func setOverlayView() {
        if let dataSource = dataSource {
            if let overlay = dataSource.swiftyOnboardViewForOverlay(self) {
                ...
                overlay.didTapSkip = {
                    self.delegate?.tapSkip()
                }
                overlay.didTapContinue = {
                    if overlay.continueButton.titleLabel?.text == "Get Started" {
                        self.delegate?.tapSkip()
                    }
                    
                    if (Int(self.getCurrentPosition()) + 1) < self.pageCount {
                        
                        let index = CGFloat(Int(self.getCurrentPosition()) + 1)
                        self.containerView.setContentOffset(CGPoint(x: index * self.frame.width, y: 0), animated: true)
                        if (Int(self.getCurrentPosition()) + 1) < self.pageCount {
                            overlay.skipButton.setTitle(nil, for: .normal)
